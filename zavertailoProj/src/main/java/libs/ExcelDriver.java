...
public class ExcelDriver {
	...
	public static Map getMultipleData(String dataFileName, String sheetName, int columnNumber) throws IOException {
		...
		InputStream input = new FileInputStream(dataFileName); //установить конекшин к файлу (открыть)
		// Get Excel WorkBook from input stream
		HSSFWorkbook wb = new HSSFWorkbook(new POIFSFileSystem(input)); // считываем данные
		// Get Excel sheet from WorkBook
		HSSFSheet sheet = wb.getSheet(sheetName); // сохранили в переменную

		// Get number of data values
        int dataSize = sheet.getPhysicalNumberOfRows() - 1; // получаем к-во заполненых строк
		// Look over the table and put key-value pairs into the Map collection
		for (int k = 1; k < (dataSize + 1); k++) {
			HSSFCell keyCell = sheet.getRow(k).getCell(0);//считали ключ
			HSSFCell valueCell = sheet.getRow(k).getCell(columnNumber);//считали значения
			keyCell.setCellType(HSSFCell.CELL_TYPE_STRING);
			valueCell.setCellType(HSSFCell.CELL_TYPE_STRING);//значение которое считываем приводим к стринге
