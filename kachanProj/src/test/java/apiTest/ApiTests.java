...
public class ApiTests {
    ...
    public void getAllPostsByUser() {
        ...
        for (int i = 0; i < responseBody.length; i++) {
            Assert.assertEquals("User name ", USER_NAME, responseBody[i].getAuthor().getUsername());
        }
...
//                new PostDTO("test2", "test body2","All Users", new AuthorDTO("autoapi"), false)
    ...
    public void getAllPostsByUserNegative(){
        ...
        Assert.assertEquals("Messge in response", "\"Sorry, invalid user requested.undefined\"", actualResponse);
        Assert.assertEquals("Messge in response", "Sorry, invalid user requested.undefined", actualResponse.replace("\"",""));
    }
    @Test
    public void getAllPostsByUserPath(){
        ...
        for (int i = 0; i < actualAuthorList.size(); i++) {
            softAssertions.assertThat(actualAuthorList.get(i).get("username")).as("Item number "+ i).isEqualTo(USER_NAME + 1);
